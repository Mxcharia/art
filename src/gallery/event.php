...
if (isset($_GET['id'])) {
  ...
  $exhibit_id = filter_input(INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT);
  // Fetch art details from the database
  $event_details = $services->selectwhere("exhibit", "id", '=', $exhibit_id);
  $event = mysqli_fetch_assoc($event_details);
} else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($response == 0) {
    ...
    $services->updateEvent($exhibit_id, $values);
    $success = "event upate was successful";
  } else {
    $error = $response;
  }
}
  ...
  <div class="form-container">
    ...
    <form method="post" autocomplete="off" enctype="multipart/form-data">
      ...
      <div class="form-group">
        ...
        <input type="text" id="name" class="form-control" name="name" placeholder="Enter event Name" value="<?php echo isset($event['name']) ? $event['name'] : ''; ?>" />
        <span class="error" aria-live="polite" id="email-error"></span>

      </div>
      <div class="form-group">
        <label for="description">Event Description:</label>
        <textarea id="description" name="description" placeholder="Enter event description ..." class="form-control"> <?php echo isset($event['description']) ? $event['description'] : ''; ?></textarea>
        <span class="error" aria-live="polite" id="email-error"></span>
      </div>
      <div class="form-group">
        <label for="price">Ticket Price:</label>
        <input type="number" id="price" class="form-control" name="price" placeholder="Enter event ticket price" min="1" value="<?php echo isset($event['price']) ? $event['price'] : ''; ?>" />
        <span class="error" aria-live="polite" id="email-error"></span>
      </div>
      <center>
